...
describe("Context Resolver", async () => {
    ...
    test('fromModule: shop does not exist', async () => {
        expect(contextResolver.fromModule(new Request('https://example.com/?shop-id=test&shopware-shop-signature=aaa'))).rejects.toThrowError('Cannot find shop by id test')
    });

    test('fromModule: missing header', async () => {
        expect(contextResolver.fromModule(new Request('https://example.com/?shop-id=blaa'))).rejects.toThrowError('Missing shopware-shop-signature query parameter');
    });

    test('fromModule: shop exists', async () => {
        app.signer.verifyGetRequest = jest.fn().mockResolvedValue(true);

        const context = await contextResolver.fromModule(new Request('https://example.com/?shop-id=blaa&shopware-shop-signature=aaa'));
    ...
    test('fromSource: missing signature header', async () => {
        expect(contextResolver.fromSource(new Request('https://example.com/', {
            body: JSON.stringify({
                source: {
                    shopId: 'blaa'
                }
    ...
    test('fromSource: resolved', async () => {
        ...
        const ctx = await contextResolver.fromSource(new Request('https://example.com/', {
            headers: {
                'shopware-shop-signature': 'aaa'
            },
            body: JSON.stringify({
