...
describe("HTTP Client", async () => {
    ...
    test('get, post, put, patch, delete', async () => {
        ...
        expect(client.get('/test')).resolves.toEqual({ body: {data: 'test'}, headers: new Headers, statusCode: 200 });
        expect(client.post('/test', { data: 'test' })).resolves.toEqual({ body: {data: 'test'}, headers: new Headers, statusCode: 200 });
        expect(client.put('/test', { data: 'test' })).resolves.toEqual({ body: {data: 'test'}, headers: new Headers, statusCode: 200 });
        expect(client.patch('/test', { data: 'test' })).resolves.toEqual({ body: {data: 'test'}, headers: new Headers, statusCode: 200 });
        expect(client.delete('/test')).resolves.toEqual({ body: {data: 'test'}, headers: new Headers, statusCode: 200 });
